...
interface AniXApi {
    ...
    // Anime (public) - supports optional search query ?q=
    @GET("api/anime")
    suspend fun getAnimeList(@Query("q") query: String? = null): Response<ApiResponse<List<Anime>>>
    ...
    // Filter by genre/year/status via search endpoint with query params
    @GET("api/anime/search")
    suspend fun getByGenre(@Query("q") genre: String): Response<ApiResponse<List<Anime>>>
    ...
    // Watch History — backend exposes continue-watching for GET, watch-history for POST/DELETE
    ...
    // Ratings — backend only supports POST (upsert), no GET by animeId
    @POST("api/ratings")
    ...
    suspend fun updateAnime(@Path("id") id: Long, @Body body: Map<String, Any>): Response<ApiResponse<Anime>>
